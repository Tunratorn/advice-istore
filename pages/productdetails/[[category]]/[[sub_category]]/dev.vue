<script lang="ts" setup>
    ...
    async function getDataProduct() {
        ...
        if (JSON.parse(respon).status == "success") {
            ...
            if (data_recommend.menu_group_name.toLowerCase().replaceAll(' ','-') != category.value) {
                ...
                prodcutImg.value = data_recommend.menu_list_dtl_img
    ...
    const checkSpecProduct = (spec: String) => {
        console.log(arrProduct.value,'dev');
        console.log(arrSpecMaster.value);
        console.log(arrProcessor.value);
        console.log(arrMemory.value);
        console.log(arrStorage.value);

        let spec_seleted = ''
        if (spec == 'color') {
            spec_seleted = spec_master.find(spec => spec.spec_name === "color")?.spec_value || ""
        }
    
        let index_spec = arrSpecMaster.value.map(i => i.spec_name).indexOf(spec) + 1
        if (index_spec > arrSpecMaster.value.length) {
            index_spec = arrSpecMaster.value.length
        }
        // console.log(arrSpecMaster.value[index_spec].spec_name);
        
        for (const [key, value] of Object.entries(arrProduct.value)) {
            // console.log("key", value.spec);
            // console.log(Object.entries(value.spec).filter(([key, value]) => key === "color" && value == "Desert Titanium"));
            
            // for (const [key_spec, value_spec] of Object.entries(value.spec)) {
            //     console.log(value_spec);
            // }
        }
    }

    const getCodeProduct = (spec_name: String,spec_value: String) => {
        checkSpecProduct(spec_name)
    ...
    onMounted(async () => {    
        ...
        if (category.value == 'mac') {
            img_detail_product.value = '/images/example/detailproductmac.png'
        } else if (category.value == 'iphone') {
            img_detail_product.value = '/images/example/detailproductiphone.png'
        }
    })
</script>
                ...
                <div class="row p-4 p-lg-0 mt-3">
                    <div class="col-12 col-lg-7">
                        <div id="frmImgProduct" class="frame-img-productdetail">
                            <img v-if="prodcutImg" class="product-img-preview anime-fade-in" :src="prodcutImg" alt="product-img-detail" srcset="">
                        </div>
                    </div>
